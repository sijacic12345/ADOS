...
void RGBtoYUV420(const uchar rgbImg[], int x, int y, uchar Y_buff[], char U_buff[], char V_buff[]) 
{
	...
	double U, V;
	// TO DO
    //iteriramo kroz celu sliku za Y komponentu
    for(int j=0;j<y;j++){
        for(int i=0;i<x;i++){
            R=rgbImg[j*3*x+i*3];
            G=rgbImg[j*3*x+i*3+1];
            B=rgbImg[j*3*x+i*3+2];
            Y_buff[j*x+i]=(uchar)(0.299*R+0.587*G+0.114*B);
        }
    }

    //za U i V vrsimo pododabiranje 2x2
    for(int j=0;j<y;j+=2){
        for(int i=0;i<x;i+=2){
            ...
            for(int jj=0;jj<2;jj++){
                for(int ii=0;ii<2;ii++){
                    int currX=i+ii;
                    int currY=j+jj;
                    R=rgbImg[currY*3*x+currX*3];
                    G=rgbImg[currY*3*x+currX*3+1];
                    B=rgbImg[currY*3*x+currX*3+2];
...
void YUV420toRGB(const uchar Y_buff[], const char U_buff[], const char V_buff[], int x, int y, uchar rgbImg[]) 
{
    ...
    for(int j=0;j<y;j++){
        for(int i=0;i<x;i++){
            ...
            rgbImg[j*3*x+i*3]=(uchar)std::max(0.0,std::min(255.0,R));
            rgbImg[j*3*x+i*3+1]=(uchar)std::max(0.0,std::min(255.0,G));
            rgbImg[j*3*x+i*3+2]=(uchar)std::max(0.0,std::min(255.0,B));
        }
    }
}
